...
def auction_winner_name(item: object) -> Optional[str]:
    """Retorna o nome público do vencedor sem assumir que a relação ORM foi carregada.

    O caminho rápido de lances transforma a linha do PostgreSQL em
    ``SimpleNamespace`` para evitar hidratação ORM. Esse objeto contém
    ``winner_user_id``, mas não a relação ``winner``. Usar ``getattr`` mantém o
    payload seguro tanto para objetos ORM quanto para o objeto leve.
    """
    winner = getattr(item, "winner", None)
    return public_user_name(winner) if winner else None
...
def public_auction_payload(item: AuctionItem, db: Session, user: Optional[User] = None) -> dict:
    ...
    return {
        ...
        "winner_name": auction_winner_name(item),
...
def _auction_row_to_namespace(row) -> SimpleNamespace:
    """Converte uma linha SQL em objeto leve para o caminho quente do lance.

    Relações ORM não existem nesse formato; campos opcionais usados pelo
    payload recebem valor seguro para evitar falhas depois do commit.
    """
